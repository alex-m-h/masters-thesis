...
are countable.
% Note that here second countability is essential. Some authors do not 
...
\begin{theorem}[Partition of unity]\label{thm:partition_of_unity}
...
to our open cover. 
Let $\{ \chi_i \}_{i=1}^N$, $N \leq \infty$, be a partition of unity 
...
\begin{proof}
    ...
    % because $\det D(\boldsymbol{\psi}_j \circ \boldsymbol{\phi}_i^{-1})(x)^{-1} > 0$.
...
$D(\bm{\phi} \circ \bm{\psi}^{-1})^\top$ which is smooth, if we choose different local coordinates 
...
same orientation for integration of forms as for the volume form. 
...
As the term 'singular' suggests, these simplices can be degenerated. For example, $\sigma_k$ could 
be constant, so the object in the topological space corresponding to the 
...
%      We denote $|\cdot |$ as the standard 
%     Euclidian norm and
%     define 
%     \begin{align*}
%         \chi_R(x) \vcentcolon= 
