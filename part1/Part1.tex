...
\begin{lemma}\label{lem:1form_with_bounded_support_in_same_cohomology}
    Let $\Omega$ be an exterior domain and assume that the first cochain homology homology 
    group $H^1(\Omega)$ is generated by $[\gamma]$ i.e. $H^1(\Omega) = \mathbb{Z} [\gamma]$.
    Take now a smooth $1$-form $\theta \in C^\infty \Lambda^1(\Omega)$.
    Then there exists a $\hat{\theta} \in C^\infty_b \Lambda^1(\Omega)$  
    s.t. $\theta = \hat{\theta} + d\mu$ for some $\mu \in \Lambda^0(\Omega)$
\end{lemma}
\begin{proof}
    Take a ball $K_R$ centered at the origin with radius $R$ large enough s.t. 
    $\real^3 \setminus \Omega \subseteq B_R$. Denote 
    $A \subseteq \real^3 \setminus \overline{\Omega}$ and define 
    \begin{align*}
        C^\infty \Lambda^k(\Omega, A) 
        \vcentcolon= \{ \omega \in C^\infty \Lambda^k(\Omega) \mid \omega(x) = 0 \forall x \in A \}.
    \end{align*}
    for $k \in \naturalnum$. Note that this defines a cochain complex with the exterior derivative $d$.

    Now recall the definition of a exact sequence \ref{}. Denote 
    ...
    $\iota$ and $\rop$ are cochain maps and we also recognize that 
    the sequence 
    \begin{align*}
        0 \rightarrow \Lambda C^\infty \Lambda^k(\Omega, A) \xrightarrow{\iota}
        C^\infty \Lambda^k(\Omega) \xrightarrow{\rop} C^\infty \Lambda^k(A)
    \end{align*}
    is an exact sequence. In this situation we can use the Snake Lemma \cite[???]{topology_and_geometry}
    to get a long exact sequence on the level of cohomology of which we take the following excerpt
    \begin{align*}
        H_{dR}^1(\Omega,A) \xrightarrow{[\iota]} H_{dR}^1(\Omega) \xrightarrow{[\rop]} H_{dR}^1(A).
    \end{align*}
    Now because $\mathbb{S}_R$ the sphere with radius $2$ is a deformation retract of $A$
    we have $H_1(A) = H_1(\mathbb{S}_R) = 0$. The exactness now implies that 
...
\begin{proof}
    ...
    \end{align*}
    We define $\bm{\phi} \in C_b^\infty(\overline{\Omega};\real^3)$ by restricting $\tilde{\bm{\phi}}$ to $\Omega$. 
    ...
    \end{align*}
    with some $\psi_R \in H^1(\Omega_R)$. e also know that $\mathbf{B}_R$ is smooth because 
    it is curl and divergence free from Thm.\ref{thm:smoothness_of_solutions}. 
    Because $\mathbf{B}_R$ and $\bm{\phi}_R$ are smooth
    ...
    is curl as well as divergence free. Hence, $\mathbf{B}$ is a solution to our problem \ref{}.
\end{proof}
