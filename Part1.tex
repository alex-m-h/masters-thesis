...
\end{align*}
$Lp^k(\Omega)$ are the spaces of $k$-forms 
...
Before we formulate the boundary value problem that we will study we have to 
...
\begin{definition}[Boundary]
    ...
    simplex without vertex $\hat{x}_{i_j}$. We then extend the definition
...
\cite{}. %%%TBD: Are you sure?


Next, we define the space of \textit{$k$-cochains} $C^k(K)$ which is nothing else 
then the dual space of the space of $k$-chains i.e.
\begin{align*}
...
In order to show existence and uniqueness of solutios of the magnetostatic 
...
\begin{proof}
    ...
    $[\gamma]$ to be a generator of the homology space. Using this
    ...
    %%% TBD: I am not sure if we need \lambda integer or real
...
being zero corresponds to $\omega \in \mathring{W}^2_2(\Omega)$.\cite{}. 
...
a 2-form $\omega \in \mathring{W}_2^2(\Omega)$:

\begin{align}
    \delta \omega &= 0, \\ 
    d\omega  &= 0 \text{ in } \Omega.
\end{align}
Next, we have to add the integral condition. We use
$**=(-1)^{k(n-k)}\tilde{\nu} = \tilde{\nu}$ \cite[p.66]{arnold} for any
differential form $\tilde{\nu}$.
and observe
\begin{align*}
...
curve can is equivalent to the curve integral of the associated vector field
(cf. \cite[6.2.3]{arnold}). Hence, we can add the integral condition 
\begin{align}
...
solution. Take a closed cochain $F \in C^k_p(K)$ s.t. $F(\gamma) = C_0$ 
and 
$F(\partial d) = 0$ for $(k-1)$-chains $d$ 
...
and a harmonic $\omega \in \mathcal{H}^2$ s.t. %%%TBD: Define the L2 spaces
\begin{align*}
...
Using the factthat the range of $\delta$ is closed on bounded domains 
