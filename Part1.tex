...
\begin{align*}
    ...
    \\ &= \star ^{-1} (F_{*,p})^* \rho(F(p)) dx^1 \wedge ... \wedge dx^n
    \\ &= \star ^{-1} \rho(F(p)) \det DF(p) \, d\hat{x}^1 \wedge ... \wedge d\hat{x}^n
...
\begin{align*}
    ( \star^{-1} F^* \star X^\flat)^\sharp 
    =\vcentcolon \hat{X} = \sum_i \hat{X}_i(\hat{x})  
...
\end{align*}
which we recognize with the Piola transform again.
...
countable oriented atlas $\{ (U_i, \phi_i) \}_{i=1}^N$ with $N \leq \infty$. 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \\ &= \sum_{r,s=0}^n \frac{\partial(\phi_i \circ \psi_j^{-1})_r}{\partial y_k}(\psi(p))
            \, \frac{\partial(\phi_i \circ \psi_j^{-1})_s}{\partial y_l}(\psi(p))
...
\begin{example}[De Rham complex]\label{ex:de_rham_complex}
...
of differential forms, the de Rham cohomology (\ref{ex:de_rham_complex}). 
...
\begin{proof}
    ...
    and thus $\lVert l \rVert _{H^(\Omega)'} \leq \lVert g_l \rVert _{H^-1/2}$ {\color{red} Wrong!}. 
    ...
    \begin{align*}
        \langle g_l, \mu \rangle = \langle l, \bar{f} \rangle 
        \leq \lVert l \rVert _{H^1(\Omega)'} \lVert  \bar{f} \rVert _{H^1}
...
\begin{theorem}[Smoothness of solutions]\label{thm:smoothness_of_solutions}
    ...
    $B \in H(\diver;\Omega) \cap H(\curl;\Omega)$ and 
    \begin{align*}
        \curl B &= 0,
        \\ \diver B &= 0.
    \end{align*}
...
\begin{proof}
    ...
    i.e. by recalling the musical isomorphism \ref{}
     $\tilde{\theta}^\sharp = \tilde{\phi} \in C^\infty(\Omega_\epsilon)^3$.
    ...
    We now restrict $\phi$ further to
    ...
    integral did not change. We know from \ref{} 
    that $\grad H^1(\Omega_R)$ is closed in $L^2$ because $\Omega_R$ is bounded. 
    So we get that 
    \begin{align*}
        \lim\limits_{i\rightarrow \infty}\nabla \psi_i = \nabla \psi_R
    \end{align*}
    with some $\psi_R \in H^1(\Omega_R)$. Because $B_R$ and $\phi_R$ are smooth
    ...
    Notice that because $B \in \mathfrak{H}^1$ it already satisfies 
...
\begin{proof}
    ...
    \end{align*}
    because $B, \tilde{B} \in \mathfrak{H}^1$.
    Thus, $B = \tilde{B}$ which concludes the proof of uniqueness.
\end{proof}
