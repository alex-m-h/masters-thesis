...
\begin{definition}
    ...
    respectively. We write a basis in round brackets $(\cdot)$ if the basis is 
...
We want to emphasize the special case of the pullback of an alternating $n$-linear map 
...
\end{proposition}
\begin{proof}
    Let denote with $\vol'$ the dual basis of $V'$ corresponding of $\vol$ i.e.
...
The pullback of $n$-linear alternating forms in terms of vector proxies is 
...
subspace topology i.e. we a set $V \subseteq \real_- \times \real^{n-1}$ is
...
The definition of differentiabilty is independent of the chart. 
...
\begin{align*}
    ...
    = \boldsymbol{\tilde{\psi}} \circ \psi^{-1} \circ \psi \circ F \circ \phi^{-1} 
        \circ \phi \circ \boldsymbol{\tilde{\phi}}^{-1}
\end{align*}
...
For that define $\gamma \vcentcolon= \boldsymbol{\phi}^{-1} (\boldsymbol{\phi}(p) + \mathbf{v}t)$.
Then
\begin{align*}
...
\begin{align*}
    0 = \left( \sum\limits_{i=1}^n \lambda_i \frac{\partial}{\partial x_i}
        \right) (x_j)
    = \sum\limits_{i=1}^n \lambda_i \frac{\partial (x_j \circ \phi^{-1}}{\partial x_i}(\phi(p))
    = \lambda_j
\end{align*}
...
\begin{proposition}
    ...
    $p$ and $F(p)$ of the charts $\phi$ and $\psi$ and let $\frac{\partial}{\partial x_i}|_p$ and 
...
the chart transition $D(\psi \circ \boldsymbol{\phi}^{-1})(\boldsymbol{\phi}(p))$.
...
the topology on $M$. Note that this means that for a manifold with boundary 
an $\omega \in \smoothcompforms{k}{M}$ are not necessarily zero on the boundary.
...
We can now identfy every vector field with a $1$-form or 
...
\end{align*}
which we identify with  $\grad f$. In other words, 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \\ &= \det G^{(i)}_{\boldsymbol{\phi}_i^{-1}(x)} \,
            \left( \det D(\boldsymbol{\psi}_j \circ \phi_i^{-1})(x)^{-1}\right)^2.
    \end{align*}
...
\end{align}
the $i$-th face map which we denote by $F^k_i$ and sometimes the $^k$ will be left out. 
...
\begin{example}
    If we set $C_k(X) = {0}$ for $k < 0$ then the groups of $k$-chains are a chain complex together with 
...
\begin{definition}[Chain map]
    ...
    $f_{i-1} \circ \partial = \partial \circ f_i$ i.e. the following diagram is commutative
    \begin{equation*}
        \begin{tikzcd}
            ... \arrow[r, "\partial"] 
...
the pullback we observe for a singular $k$-simplex $\sigma$ in $M$.
Now, 
...
\end{theorem}
This is quite a deep result that requires more results than we introduced ...
...
operators $\curl$, $\grad$ and $\diver$ on an open domain $\Omega$. We will later 
introduce some assumptions on the boundary of it. Throughout the 
...
\begin{definition}[Weak divergence]
    Let $\Omega \subseteq \real^3$ be open. For $\mathbf{v} \in L^2(\Omega)^3$ we the  
    Define the space
    \begin{align*}
...
\begin{proof}
    ...
    $\gamma$ is $\gamma'$ is injective. Thm.\,2.19 in the same reference 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \leq C \lVert g_l \rVert _{H^-1/2} \, \lVert u \rVert _{H^1} 
    \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        \int_\Omega u \, \diver \mathbf{v} \, dx 
        + \int_\Omega \grad u \cdot \mathbf{v} \, dx 
        = \langle \gamma_n , u \, \rangle \quad \forall u \in H^1(\Omega).
    \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        = \langle g, \tr \phi \rangle_{H^{-1/2}(\partial \Omega)} \quad \forall \phi \in H^1(\Omega_R).
    \end{align*}
...
$Q$ being "bounded away from the boundary".
...
\begin{proof}
    ...
    $C^\infty_0(\Omega;\real^3)$ is dense in $H_0^1(\Omega;\real^3)^3$ we can find 
...
\begin{remark}
    This also works for $\Omega$ open without regularity assumtpoi
\end{remark}
...
\begin{proof}
    ...
    of $\tilde{\bm{\phi}}$ using Example \ref{ex:integration_1_forms} the connection 
    of the curl with the exterior derivative to get 
    \begin{align*}
        \int_\gamma \tilde{\theta} &= \int_\gamma \tilde{\bm{\phi}} \cdot dl = C_0 
...
\begin{proof}
    ...
    if $\phi \notin L^2$ it is due to its behavious at infinity.
