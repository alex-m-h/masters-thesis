...
\begin{proposition}\label{integral_exact_form_zero}
    Let $(\phi_i)_{i\in\mathbb{N}} \subseteq H^0(d)$ s.t. $(d\phi_i)$ 
    is convergent and let $\gamma \in C_1(K)$ be a bounded $1$-chain.
    Then
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        = \bar{I}(\nu)(\gamma)
    \end{align*}
    where we used the properties of $\rop$. Let $\tilde{\Omega}$ be a bounded
    subdomain of $\omega$.
    Because the image of $d$ is closed for every bounded domain 
    (cf. \cite[Lemma 7]{picard}) we have for any convergent sequence 
    $(d\psi_i)$ exists a $\psi \in H^0(d;\tilde{\Omega})$ s.t.
    \begin{align*}
        \lim\limits_{i \rightarrow \infty}d\psi_i| _{\tilde{\Omega}} 
        = d\tilde{\psi}.
    \end{align*}
    Then due to the continuity of $\rop$
    \begin{align*}
...
Now we construct a solution. Let us denote 
$\Omega_R \vcentcolon= \Omega \cap B_R$ with $R>0$ large enough 
s.t. $\gamma \subseteq \tilde{\Omega}$. Then we can know there exists
a smooth closed $1$-form $\nu$ defined on $\Omega_R$ 
s.t. $\int_\gamma \nu = (\bar{I}\nu)(\gamma)$ \cite{}. Because the trace of 
$\nu$ on the exterior boundary of $B_R$ is smooth we can now extend this by 
solving the boundary problem on $\real^3 \setminus B_R$
\begin{align*}
    d\chi &= 0 \text{ in } \real^3 \setminus \overline{B}_R \\ 
    \text{tr}\,\chi &= \text{tr}\,\nu \text{ on } \partial B_R.
\end{align*}
This problem has indeed a solution (cf. \cite{schwarz}).
We obtain a $1$-form $\bar{\nu} \in H^1(d;\Omega)$ s.t. $d\bar{\nu} = 0$
and $\bar{I}(\nu)(\gamma) = C_0$. $*\bar{\nu}$ is now a $2$-form with 
$\delta*\bar{\nu} = 0$ i.e. it is co-closed. We use the Hodge decomposition 
(\refeq{hodge_decomposition}) and project $*\bar{\nu}$ onto 
$\mathring{H}^2_0(d,\delta;\Omega)$ to obtain $\omega$.
Thus we have $d\omega = 0$, $\delta\omega=0$ and the boundary conditions are
fulfilled. What remains is to show the integral condition. We have using 
Prop.\,\ref{integral_exact_form_zero}
\begin{align*}
    \bar{I}(*\omega)(\gamma) = 
    \bar{I}(*(*\bar{\nu} - \lim\limits_{i\rightarrow\infty}\delta\phi_i))
    (\gamma)=
    \bar{I}(\bar{\nu} \pm \lim\limits_{i\rightarrow\infty}d\phi_i)
    (\gamma)=
    \bar{I}(\bar{\nu})(\gamma) = C_0.
\end{align*}
So the integral condition is also fulfilled and thus 
...
\end{theorem}
\begin{proof}
    Let $\omega$ and $\tilde{\omega}$ both be solutions. Then 
    $c \mapsto \bar{I}(*\omega)(c)$, 
    $c \mapsto \bar{I}(*\tilde{\omega})(c)$ are both 
    closed $1$-cochains in $H^1(K)$. Then we know from 
    (\ref{uniqueness_cochain}) that they are in the same cohomology class 
    and so $\bar{I}(\omega)(\cdot) - \bar{I}(\tilde{\omega})(\cdot) = \delta J$.
    We can now use the techniques from \cite{goldshtein} 
    {\color{red}TBD: More rigorous argument.} to get that 
    $*\omega - \tilde{*\omega} = d \theta$ with $\theta \in W^0_{2,loc}$. 
    By applying the Hodge star operator we get 
    $\omega - \tilde{\omega} = \delta \mu$ with $\mu \in W^3_{2,loc}$
    That means 
    ...
    Because we have $B, \tilde{B} \in \mathring{H}(\diver)$ %TBD: Not defined
