...
\begin{proof}
    ...
    \begin{align*}
        ...
        \right) (\gamma) \right| = 0
    \end{align*}
    because $\epsilon$ was arbitrarily small.
...
every vertex from $\gamma$ does not touch the boundary of $L$. Let $\rop_L$ be 
the regularization operator from Thm.\,\ref{operators}, but now constructed for
$L$ instead of $K$.

Take a cochain $F \in C^1(L)$ s.t. $F(\gamma) = C_0$. Then we take the 
Whitney transformation of $wF$ defined at \ref{}. Because $w$ induces an 
isomorphism on cohomology $I(wF)(\gamma) = F(\gamma) = C_0$. Then denote 
$\theta \vcentcolon= \varphi wF$. By our definition of integration \ref{}
