...
We can then use $\sigma: \Delta_k \rightarrow M$ as a chart. Technically, 
we use here only the interior of $\Delta_k$, but since we are integrating over it 
and the boundary is a null-set, this does not matter for the argument. Now, 
...
\begin{align*}
    ...
    = \sum_{i=0}^{k} (-1)^i \int_{\Delta_{k-1}} F_i^*\sigma^* \omega
...
\begin{proof}
    ...
    It is then trivial to see from the definition of $R_{Y,X}\Gamma(T^*)$
