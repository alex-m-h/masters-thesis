...
For the discretization we will use the space of pushforward tensor product splines 
...
since we use the same method as presented in this paper and also to fix notation. 
...
reference domain and then use a pullback and pushforward to transfer our degrees of freedom 
and basis functions to the physical domain.
...
points is well-defined (see \cite[p.30 Sec.\,3.3.1]{isogeometric analysis}).
...
\begin{align*}
    \hat{\mathtt{n}}_\mathbf{i} \vcentcolon= (\zeta_{i_1}, \zeta_{i_2}), \quad \mathbf{i} \in \mathcal{M}^0
...
These degrees of freedome are unisolvent i.e. with $N_l = |\mathcal{M}^l|$ 
with some ordering $\mu_0, \mu_1, ..., \mu_{N_l}$ of the indices of $\mathcal{M}^l$ we
define 
\begin{align*}
    \bm{\sigma}^l \vcentcolon= (\sigma^l_{\mu_0}, \sigma^l_{\mu_1}, ..., \sigma^l_{\mu_{N_l}})^\top
...
it can not be embedded into the continuous functions. Thus, 
...
pushforward of the basis functions on the reference domain. We will now clarify what 
...
We already introduced the pullback in Sec.\ref{}. In the case of two dimensions 
