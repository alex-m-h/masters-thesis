...
homogeneous boundary conditions. Then we assume $\psi \in \bar{V}_h^0$. 
This has the consequence that for $\psi, \tilde{\psi} \in \bar{V}_h^0$ fulfilling the 
previous conditions at (\ref{eq:conditions_psi}) for two possibly different curves 
$\Gamma$ and $\tilde{\Gamma}$, then 
...
We assume additionally $\veccurl \psi \in V_h^1$. Since 
we can choose $\psi$ and we assume to be on a discretizable domain, these 
...
$\Gamma_1$ and $\Gamma_2$ with $\psi_1$ and $\psi_2$ accordingly and 
...
\begin{align*}
    B^p_{i+1}(x) \vcentcolon= \frac{x - \xi_i}{\xi_{i+p}- \xi_i} B_i^{p-1}(x) 
        + \frac{\xi_{i+p+1} - x}{\xi_{i+p+1}- \xi_{i+1}} B_{i}^{p-1}(x) 
\end{align*}
and 
\begin{align*}
...
\end{align*}
numerically. In order to do that, we interpolate $\psi$. Because $\psi$ is not 
