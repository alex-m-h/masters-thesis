...
We start in Sec.\,\ref{sec:variational_formulation_of_the_magnetostatic_problem_in_2D} 
by deriving a variational formulation of the 2D magnetostatic problem, 
including the alternative description of the curve integral constraint,
and prove well-posedness of the resulting formulation. In Sec.\,\ref{sec:discretized_magnetostatic_problem}, 
the discretization 
of the problem will be described including a proof of well-posedness and of an a-priori
estimate. The implementation will be explained in Sec.\,\ref{sec:implementation} and numerical examples 
given in Sec.\ref{sec:numerical_examples} which confirm our theoretical predictions.
...
\section{Variational formulation of the magnetostatic problem in 2D}\label{sec:variational_formulation_of_the_magnetostatic_problem_in_2D}
...
\subsection{The \texorpdfstring{$\bm{\curl}$-$\diver$}{curl-div} Hilbert complex}\label{sec:curl_div_hilbert_complex}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \fill[fill=green] (0,0) circle [radius=4];
        \draw[dashed, -{Stealth[length=3mm,width=3mm]}, blue] (4,0) 
            arc [start angle=0, end angle=180, radius=4];
        \draw[dashed, -{Stealth[length=3mm,width=3mm]}, blue] (-4,0) arc [start angle=180, end angle=360, radius=4] ;
        \draw[fill=white] (0,0) circle [radius=2];
        \node[above] at (45:5.25) {\large$\partial \Omega_{out}$};
        \node[below] at (45:1.75) {\large$\partial \Omega_{in}$};
        \node at (45:3.7) {\color{blue} \Large$\Gamma$};
        \node at (135:3) {\color{green!30!black!100}\Large$\Omega_\Gamma$};
        \node at (90:5.5) {\Huge$\Omega$};
    \end{tikzpicture}
    }
...
choose a suitable variational formulation. This variational formulation 
...
\begin{proposition}\label{prop:equivalence_first_two_equations}
    ...
    (\ref{eq:second_eq_mixed_formulation}) hold iif 
...
Let $\mathbf{n}$ be the unit outward normal 
of $\Omega_\Gamma$ and $\bm{\gamma}$ the parametrization of $\Gamma$. Then we know that $\mathbf{n} \perp \bm{\gamma}'$
and
\begin{align*}
...
\end{align}
We call such a $\psi$ \textit{admissible for $\Gamma$}.
Then we observe 
\begin{align*}
...
However, $\psi$ is of course not uniquely determined by (\ref{eq:conditions_psi}) 
so we have to check what happens when we use a different admissible $\psi$ respecting these conditions.
...
\begin{proposition}\label{prop:different_psi_same_curve_integral}
    ...
    i.e. they are both admissible for $\Gamma$,
...
\begin{proposition}\label{prop:different_curve_same_curve_integral_substitute}
    Let $\Gamma_1$ and $\Gamma_2$ be chosen as just described and admissible $\psi_1$ 
    and $\psi_2$ respectively. Let $\sigma \in H^1_0$, $\mathbf{B} \in H_0(\diver)$ 
...
\begin{proof}
    ...
    $\psi_1 - \psi_2 = 0$ on $\partial\Omega_{out}$. From the admissibility (\ref{eq:conditions_psi}), 
    we also get 
    ...
    (\ref{eq:second_eq_mixed_formulation}), $J=0$ in $\Omega_{\Gamma_1}\cup \Omega_{\Gamma_2}$ 
    and $\psi_1 = \psi_2 = 1$ in $\Omega \setminus (\Omega_{\Gamma_1}\, \cup\,\Omega_{\Gamma_2} )$,
    this means
    \begin{align*}
        \langle \veccurl (\psi_1 - \psi_2), \mathbf{B} \rangle 
        = \int_{\Omega \setminus (\Omega_{\Gamma_1} \cup \,\Omega_{\Gamma_2} )} (\psi_1 - \psi_2) \, J \, dx
        = 0.
    \end{align*}
\end{proof}
...
\begin{theorem}[BNB]\label{thm:BNB}
    ...
    (\ref{eq:general_variational_problem}) is well-posed iif the following two criteria are fulfilled
    \begin{align}
...
(see \cite[Rem.\,25.14]{ern_guermond_2}).
...
assuming $\psi$ is admissible for $\Gamma$ i.e. (\ref{eq:conditions_psi}) holds.
...
\begin{proposition}
    ...
    projection onto the harmonic forms. Then with $\psi$ being admissible for $\Gamma$, 
    i.e. fulfilling conditions (\ref{eq:conditions_psi}),
...
\begin{proposition}\label{prop:harmonic_part_curlpsi_independent_of_choice}
    Choose $\psi$ and $\tilde{\psi}$ admissible for $\Gamma$ according to (\ref{eq:conditions_psi}). Then
    \begin{align*}
...
\begin{lemma}\label{lem:T_for_T_coercivity_surjective}
    ...
    \begin{align*}
        ...
        = \bigg(\sigma - \frac{1}{c_P^2}\veccurl^{-1}\mathbf{B}_\mathfrak{B}, \veccurl \sigma + \mathbf{B} + \lambda \beta \mathbf{p},
            \alpha \langle \mathbf{p}, \mathbf{B} \rangle  
            + \frac{\lambda}{c_\psi}\bigg).
    \end{align*}
...
\begin{theorem}
    Let $\psi$ be admissible for $\Gamma$ and 
    $\veccurl \psi_0 + c_\psi \mathbf{p} = \veccurl \psi$ and assume we have chosen the sign of 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        + \lVert  \veccurl \sigma \rVert^2 + \lVert \diver \mathbf{B} \rVert ^2
        \\ &\quad+ \lambda ^2 \left( \beta - \frac{4 c_1^2 c_P^2}{2 c_\psi^2} \right)
        + \lVert \mathbf{B}_\mathfrak{H} \rVert^2  \left( \frac{c_\psi^2}{4 c_1^2 c_P^2 }
    ...
    \begin{align*}
        ...
        \\ &\quad+ \frac{c_1^2 c_P^2}{c_\psi^2}\lambda^2
...
\begin{proof}
    ...
    first equality.
    The statement follows immediately from the previous theorem, 
...
We would like to represent it in our discretization, which leads us to the discrete Hilbert complex. 
...
$(V^k,d^k)$ and dual complex $(V^*_k,d^*_k)$ for $k \in \integers$. For this section, 
...
\end{align*}
We will later investigate the difference between the spaces of discrete and continuous 
...
% \end{align}
% for some generic constant $C>0$ independent of $v$ and $h$, the mesh size $h$ and some 
...
% \end{proposition}
% \begin{proof}
%     At first note, that for any $z \in \mathfrak{B}^j$, $\lVert z \rVert _V = \lVert z \rVert$.
%     Since $\mathfrak{B}^j_h \subseteq \mathfrak{B}^j$, $d(z_h, \mathfrak{B}^j) = 0$ for any 
%     $z_h \in \mathfrak{B}^j_h$. 
...
\begin{align*}
    \weakcurl_h \mathbf{B}_h = J_h \text{ and }
    \diver \mathbf{B}_h = 0
\end{align*}
with $J_h \in V_h^0$ plus the additional curve integral constraint. 
...
\begin{problem}\label{prob:magnetostatic_problem_discrete_variational}
    ...
    \end{align*}
\end{problem}
In practice, $\mathbf{p}_h$ 
is computed numerically before assembling the system. 
Now we make some assumptions on $\psi$. Assume there exists a discrete space $\bar{V}_h^0 \supseteq V_h^0$ 
s.t. $\bar{V}_h^0 \cap H^1_0 = V_h^0$. $\bar{V}_h^0$ will correspond to the discrete space without 
boundary conditions. 
\begin{definition}\label{def:discreteley_admissible}
    We call $\psi$ \textit{discretely admissible} for $\Gamma$ if it is 
    admissible for $\Gamma$, i.e. fulfilling (\ref{eq:conditions_psi}), and 
    additionally
    \begin{align}
        \psi \in \bar{V}_h^0 \text{ and } \veccurl \psi \in V_h^1.
    \end{align}    
\end{definition}

This has the consequence that for $\psi, \tilde{\psi} \in \bar{V}_h^0$ being discretely admissible for $\Gamma$ 
and $\tilde{\Gamma}$ respectively, 
then 
$\psi - \tilde{\psi} \in H^1_0$ and thus 
\begin{align}
    \psi - \tilde{\psi} \in \bar{V}^0_h \cap H^1_0 = V_h^0. \label{eq:psi_minus_psi_tilde_in_Vh0}
\end{align}

Also, we can apply the discrete Hodge decomposition, since $\veccurl \psi \in V^1_h$, 
to get $\psi_{0,h} \in V_h^0$, $c_{\psi,h} \in \real$ s.t.
\begin{align}
    \veccurl \psi = \veccurl \psi_{0,h} + c_{\psi,h} \mathbf{p}_h. \label{eq:split_psi_h}
\end{align}
...
\begin{theorem}[Well-posedness of the discrete problem]\label{thm:wellposedness}
    ...
    Assume $\psi$ to be discretely admissible for $\Gamma$ (cf. Def.\,\ref{def:discreteley_admissible}). 
    % Then we find 
    ...
    Split $\psi$ according to (\ref{eq:split_psi_h}). Then $c_{\psi,h}>0$ 
    does not depend on the choice of $\psi$.
    Choose $c_{1,h} > 0$ s.t. $\lVert \veccurl \psi_{0,h} \rVert \leq c_{1,h}$.
    ...
    $c_{P,h}$ instead of $c_P$, $c_{\psi,h}$ instead of $c_{\psi}$ and $c_{1,h}$ instead of $c_1$.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Quasi optimal a-priori estimate]
    Let the same assumptions hold as for Thm.\,\ref{thm:wellposedness}
    and let $(\sigma,\mathbf{B},\lambda) \in X$ be 
...
choose the discrete space $V_h^0$ instead of $H^1_0$ where we also need 
(\ref{eq:psi_minus_psi_tilde_in_Vh0}).
We use the discrete formulation of the problem, i.e.
...
\end{align}
assuming $\psi$ is discretely admissible for $\Gamma$,
...
invariance if $J=0$ between the curves and the inner boundary, i.e. if we have two different curves 
$\Gamma_1$ and $\Gamma_2$ with $\psi_1$ and $\psi_2$ discretely admissible for them and 
$J=0$ in $\Omega_{\Gamma_1} \cup \Omega_{\Gamma_1}$, then we would like
\begin{align*}
...
instead, $V_h^0$ instead of $H^1_0$ and (\ref{eq:psi_minus_psi_tilde_in_Vh0}).
...
are denoted as $B_{i,\xi}^{q_1}$, $i=0,...,n_1 - 1 = N + q_1 - 1 $, and $B_{j,\eta}^{q_2}$, 
$j = 0,...,N + q_2 - 1$, and the spaces
...
It is well-known that for a piecewise smooth function $\mathbf{v}$,  
$\mathbf{v} \in H(\diver)$ iif the normal trace across element interfaces
agrees. Analogously, a piecewise smooth $\tau \in H^1$ iif the values agree 
...
of the indices of $\mathcal{M}^\ell$, denoting $\hat{\sigma}^\ell_{\mu_i} = \hat{\sigma}_i^\ell$,  we
define 
\begin{align*}
...
\begin{align*}
    ...
    \\  W^1_d(\hat{\Omega}) &\vcentcolon= \{ v \in L^1(\hat{\Omega}) \mid \partial_d v
        \in L^1(\hat{\Omega}) \} \quad\text{for } d\in \{1,2\}
\end{align*}
and then denote $\hat{U}_{L^1}^0 \vcentcolon= W^1_{1,2}(\hat{\Omega})$, 
$\hat{U}_{L^1}^1 \vcentcolon= W^1_1(\hat{\Omega}) \times W^1_2(\hat{\Omega})$
and $\hat{U}_{L^1}^2 \vcentcolon= L^1(\hat{\Omega})$. Then the degrees of freedom $\hat{\sigma}_\mu^\ell$ are 
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[gray] (2,0) -- (2,5);
        \draw[gray] (0,0) -- (3,0);
        \draw[gray] (0,1.6666) -- (3,1.6666);
        \draw[gray] (0,3.3333) -- (3,3.3333);
        \draw[gray] (0,5) -- (3,5);
        \node at (1.5,5.8) {\Huge$\hat{\Omega}$};
        \node at (-0.5,2) {\small$\partial \hat{\Omega}_{in}$};
        \node at (3.6,2) {\small$\partial \hat{\Omega}_{out}$};
        \node at (3.4,-0.3) {\footnotesize$\hat{x}_1$};
        \node at (-0.3,5.4) {\footnotesize$\hat{x}_2$};
    \end{tikzpicture}
    \end{minipage}
    \begin{minipage}[c]{.15\textwidth}
        \scalebox{1.5}{
        ${\huge\xrightarrow{\mathbf{F}}}$
        }
    \end{minipage}
    \begin{minipage}[c]{.4\textwidth}
        \begin{tikzpicture}
            \draw[gray] (0,0) circle [radius=2];
            \draw[gray] (0,0) circle [radius=2.5];
            \draw[gray] (0:1.5) -- (0:3);
            \draw[gray] (120:1.5) -- (120:3);
            \draw[gray] (240:1.5) -- (240:3);
            \draw[very thick] (0,0) circle [radius=3];
            \draw[very thick] (0,0) circle [radius=1.5];
            \node at (0,3.5) {\Huge$\Omega$};
            \node at (315:1.05) {\small$\partial \Omega_{in}$};
            \node at (315:3.5) {\small$\partial \Omega_{out}$};
        \end{tikzpicture}
    \end{minipage}
    \caption{
        Reference domain $\hat{\Omega}$ and physical domain $\Omega$ with grid.
        We have three grid cells in both dimensions, i.e. $N=3$.
        Here $\hat{\Omega}$ has no boundary at the "northern" and the "southern" edge.
    } \label{fig:reference_domain_physical_domain}
\end{figure}

See Fig.\,\ref{fig:reference_domain_physical_domain} for an example where $\Omega$ 
is an annulus and $N=3$.
...
\begin{remark}
    ...
    \allowbreak\in \bar{V}_h^0$ and analogous for the other function spaces,
...
iif $\sigma_h \in V_h^0$, $\mathbf{B}_h \in V^1_h$ 
...
We will often from now implicitly use some flattening of the multiindices in $\mathcal{M}^\ell$.
...
\begin{align*}
    \underline{\tau}^\top (\mathbb{I}^0-\mathbb{P}^0) \mathbb{M}^0 
        ...
        \underline{\mathbf{B}} &= \underline{\tau}^\top \mathbb{P}^0 \underline{\tilde{J}} 
...
\begin{align*}
    (\mathbb{I}^0-\mathbb{P}^0)^\top \mathbb{M}^0 
        ...
        \underline{\mathbf{B}} &=  \mathbb{P}^0 \underline{\tilde{J}} 
    \\ \mathbb{P}^1 \mathbb{M}^1 \mathbb{C} \mathbb{P}^0 \underline{\sigma}
        + (\mathbb{I}^1-\mathbb{P}^1) \mathbb{M}^1 
        (\mathbb{I}^1-\mathbb{P}^1)\underline{\mathbf{B}}
...
\begin{proposition}
    ...
    \begin{align*}
        \bar{L}^1_h = -\widetilde{\bm{\grad}}_h \diver P^1_h + \veccurl P ^0_h \widetilde{\curl}_h
            + (I - P^1_h)^* (I-P^1_h)
    \end{align*}
...
We will consider two numerical examples, the simulation of the magnetic 
field induced by a current through a wire, where the exact solution is given by the Biot-Savart law, and 
a manufactured solution. The Biot-Savart solution will be approximated on a standard annulus 
and a "distorted annulus". In both cases, we will pose the problem with two different curves 
and investigate the convergence error.

\subsection{Magnetic field induced by current through a wire}
...
\begin{align*}
    \mathbf{B}(x) = \frac{\mu_0 I}{4\pi} \int_{-\infty}^\infty  
        \frac{\bm{\ell}' \times (x - \bm{\ell}(s))}{|x - \bm{\ell}(s)|^3} \, ds
...
the curve $\Gamma_1$, i.e. in logical coordinates 
\begin{align}
    \psi(\hat{x}_1,\hat{x}_2) = \begin{cases}
        2 \hat{x}_1 & \text{ for $\hat{x}_1 \leq 0.5$} 
        \\ 1 & \text{ else}.
    \end{cases}\label{eq:linear_interpolation_psi}
\end{align}Then $\psi_1$ is admissible for $\Gamma_1$ (cf. (\ref{eq:conditions_psi}))
...
we chose $64$ cells for our grid on the reference domain in both dimensions for $p=2$. 
...
of $h^{p}$ \cite[Ch.4, (4.48)]{splines_and_pdes}, if the solution is at least 
$H^{p+1}$-regular,
...
\begin{figure}
    ...
    \caption{Solution of magnetostatic problem induced by current $I = 2 \pi / \mu_0$ through a wire on an annulus for 
...
\begin{tikzpicture}[scale=0.7]
    \begin{loglogaxis}[
        title=$L^2$-error on annulus,
        xlabel=$N$,
        ylabel=$L^2$ error,
        legend entries={$p=2$, $p=3$, $0.1\times 1/N^2$, $0.1\times 1/N^3$},
        xtick={8,16,32,64},
    ...
    \addplot table [x=n_cells, y index=0] {l2_error_data/biot_savart_annulus/degree3/l2_error.csv};
    \addplot [
        domain=8:100,
        samples=50,
        dashed,
...
\begin{tikzpicture}[scale=0.7]
    \begin{loglogaxis}[
        title=$L^2$-error on distorted annulus,
        xlabel=$N$,
        ylabel=$L^2$ error,
        legend entries={$p=2$, $p=3$, $100 \times 1/N^2$, $2500 \times 1/N^3$},
        xtick={8,12,16,24,32,48,64},
        xticklabels={8,12,16,24,32,48,64}
    ]
    \addplot table [x = N, y = l2_error]{l2_error_data/biot_savart_distorted/degree2/l2_error_data.csv};
    \addplot table [x = N, y = l2_error]{l2_error_data/biot_savart_distorted/degree3/l2_error_data.csv};
    \addplot [
        domain=8:100,
        samples=50,
        dashed,
...
\caption{Convergence analysis for the approximation of the Biot-Savart solution with $p=2$ and 
    $p=3$
    on the annulus domain and the distorted annulus domain. For the solution with $p=3$, the 
    number of grid points in $\hat{x}_2$-direction is $N/2$ to avoid memory issues.
}
...
on a "distorted annulus" (see \ref{fig:distorted_annulus_domain}) using the same reference domain, but the different mapping 
\begin{align*}
    \mathbf{F}(\hat{x}) = \begin{pmatrix}
        3(\cos^2(\hat{x}_2 ) + 1 )(\hat{x}_1 + 1)\cos(\hat{x}_2 ) 
...
$\mathbf{B}_{h,g}$, that we compute before, and then split
...
$-\langle J, P^0_h \tau_h \rangle + \langle \mathbf{B}_{h,g}, \veccurl P_h^0 \tau_h \rangle$ in 
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        \filldraw[domain=0:6.28, smooth, samples=500, fill=gray, draw=black]  
            plot ({3*(cos(\x r)^2 + 1)*2*cos(\x r)},{(cos(\x r)^2 + 1)*2*sin(\x r)}) --cycle;
        \draw[domain=0:6.28, smooth, samples=500, dashed, thick, fill=green!80, draw=blue]  
            plot ({3*(cos(\x r)^2 + 1)*1.5*cos(\x r)},{(cos(\x r)^2 + 1)*1.5*sin(\x r)}) --cycle;
        \filldraw[domain=0:6.28, smooth, samples=500, fill=white, draw=black]  
            plot ({3*(cos(\x r)^2 + 1)*cos(\x r)},{(cos(\x r)^2 + 1)*sin(\x r)}) --cycle;
        \node at (6,1.125) {\color{blue}$\Gamma$};
        \node at (-6,0.875) {\color{green!40!black!100}$\Omega_\Gamma$};
...
For $p=3$, we observe again second order convergence when $\Gamma_1$ is chosen, 
but third order convergence for $\Gamma_2$. This is likely because $\psi_1$ is 
not $H^2$. Since $J \neq 0$, we have to compute 
\begin{align*}
    \int_{\Omega_{\Gamma_1}} \psi_1 \, J \, dx
\end{align*}
numerically. In order to do that, we interpolate $\psi_1$. Because $\psi_1$ is not 
$H^2$-regular, the interpolation error dominates and so we do not obtain 
third order convergence. This suggests the necessity to choose a $\psi$ 
smooth enough s.t.~it can be approximated sufficiently well.
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.8]
        \begin{loglogaxis}[
            xlabel=$N$,
            ylabel=$L^2$ error,
            legend entries={$p=2,\Gamma = \Gamma_1$\\
                            $p=2,\Gamma = \Gamma_2$\\
                            $p=3,\Gamma = \Gamma_1$\\
                            $p=3,\Gamma = \Gamma_2$\\
                            $0.1\times 1/N^2$\\
        ...
        \addplot table [x=n_cells, y=l2_error] {l2_error_data/manufactured_inner_curve/degree3/l2_error_data.csv};
        \addplot[green, mark=square*] table [x=n_cells, y=l2_error] {l2_error_data/manufactured_poisson_psi/degree3/l2_error_data.csv};
        \addplot [
            domain=8:100,
            samples=50,
            dashed,
    ...
    \caption{Convergence analysis for the manufactured solution $\mathbf{B}(x) = (|x|^2 -2) (-x_2,x_1)^\top$
        with different choices of $\psi_i$ corresponding to different curves $\Gamma_i$ as before.    }
