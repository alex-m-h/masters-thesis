...
\begin{align*}
    L^2 \xleftarrow{\curl} H(\curl) \xleftarrow{-\bm{\grad}} H^1
\end{align*}
...
\begin{proof}
    ...
    $\psi_1 - \psi_2 = 0$ on $\partial\Omega_{out}$. From the assumption we also get 
...
seen as a weak approximation of $\curl$ and the same for $\widetilde{\bm{\grad}}_h$.
...
(recall that we assume $\psi \in V_h^0$) and then we use the discrete formulation of the problem, i.e.
the first two equations of Problem\,\ref{prob:magnetostatic_problem_discrete_variational}. Then
we obtain
\begin{align}
