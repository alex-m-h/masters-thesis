...
B-Splines $B^p_i$ of degree $p\geq 1$ for $i = 0,...,n-1$ are defined recursively as 
\begin{align*}
...
and we denote
\begin{align}
     \hat{V}^0_h = \mathbb{S}_{p,p}, \; \hat{V}^1_h = \begin{pmatrix}\mathbb{S}_{p,p-1} \\ \mathbb{S}_{p-1,p}  
    \end{pmatrix}
    \text{ and } \hat{V}^2_h = \mathbb{S}_{p-1,p-1}. \label{eq:discrete_spline_reference_spaces}
\end{align}
...
define 
\begin{align*}
    \bm{\sigma}^\ell \vcentcolon= (\sigma^\ell_0, \sigma^\ell_1, ..., \sigma^\ell_{N_\ell})^\top
...
the standard choice with $\hat{V} = H^1(\hat{\Omega})$, 
...
\begin{remark}
    ...
    \in \bar{V}_h^0$ the result would be a cochain projection, but it would not be bounded and 
...
\begin{proposition}
    Define the penalized discrete Hodge Laplacian operator as
    \begin{align*}
        \bar{L}^1_h: \bar{V}^1_h \rightarrow \bar{V}^1_h,
            \mathbf{v}_h \mapsto (-\widetilde{\grad}_h \diver P^1_h + \widetilde{\curl}_h \veccurl P ^0_h
            + (I - P^1_h)^* (I-P^1_h))\mathbf{v_h}
    \end{align*}
    where $\widetilde{\grad}_h$ and $\widetilde{\curl}_h$ are the adjoints 
    of $\diver P^1_h$ and $\veccurl P^0_h$ respectively i.e. 
    defined on the full space without boundary conditions. 
    Then 
    \begin{align*}
        \ker \bar{L}^1_h = \mathfrak{H}^1_h.
    \end{align*}
\end{proposition}
Note that the spaces discrete Hilbert complex (\ref{eq:discrete_hilbert_complex}) 
...
\begin{remark}
    ...
    One way to avoid this is to use use multiple patches instead and then use a 
...
we chose $64$ cells for our grid in both dimensions and $p=2$. Note 
...
\begin{figure}
    ...
    on an annulus for 
    $64$ grid cells in both dimensions and spline degree $p=2$  }
...
goes through the origin in $z$-direction, but we will compute the resulting solution 
